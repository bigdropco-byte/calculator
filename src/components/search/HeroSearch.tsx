...
export const HeroSearch: React.FC = () => {
  ...
  return (
    <div className="w-full max-w-2xl mx-auto">
      ...
      >
        <div className="p-2 sm:p-2.5 text-slate-500 group-hover:text-sky-700 transition-colors">
        ...
        <input
          ...
          className="w-full text-base sm:text-lg text-slate-900 placeholder-slate-500 bg-transparent border-0 focus:outline-none cursor-pointer"
        />
        <span className="hidden sm:inline-flex items-center px-3 py-1.5 rounded-lg bg-slate-100 text-xs font-semibold text-slate-700 border border-slate-200 shrink-0">
          Press ⌘K
        </span>
      </div>

      {/* Examples underneath */}
      <div className="mt-3.5 flex items-center justify-center gap-1.5 flex-wrap text-xs text-slate-600">
        ...
        {examples.map((item, index) => (
          <React.Fragment key={item.slug}>
            <button
              ...
              className="hover:text-sky-700 hover:underline transition-colors font-medium text-slate-700"
            >
              {item.label}
            </button>
            {index < examples.length - 1 && <span className="text-slate-400">·</span>}
          </React.Fragment>
        ))}
      </div>
